...
def save_trajectories(args):
    ...
    for trajectory in all_filtered_trajectories:
        ...
        nan_mask = torch.isnan(trajectory)
        valid_mask = ~nan_mask.any(dim=1)
        
        if not valid_mask.any():
            continue
        
        # Find the index of the first valid point
        start_idx = valid_mask.nonzero(as_tuple=True)[0][0]
...
#            continue
        ...
        valid_trajectory = trajectory[start_idx + 1:]
        valid_indices = (valid_trajectory[:, 0].isnan() | valid_trajectory[:, 1].isnan()).nonzero(as_tuple=True)[0]
        
        if len(valid_indices) > 0:
            end_idx = valid_indices[0] + start_idx + 1
        else:
            end_idx = trajectory.shape[0]
        
        valid_trajectory = trajectory[start_idx + 1:end_idx]
        
        # Create the endpoints tensor
        endpoints = torch.arange(start_idx + 1, end_idx).unsqueeze(1)
        endpoints = torch.cat([endpoints, valid_trajectory[..., [1, 0]]], dim=1)
        
        # Append query points and endpoints
        of_query_points.append(query_point.repeat(endpoints.size(0), 1))
        of_endpoints.append(endpoints)

    of_query_points = torch.cat(of_query_points)
    of_endpoints = torch.cat(of_endpoints)

    of_point_pairs = torch.cat([of_query_points, of_endpoints], dim=-1)

    torch.save(of_point_pairs, output_path)
    print(f"Saved {output_path}, shape: {of_point_pairs.shape}")
